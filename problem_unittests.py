...
def test_get_batches(get_batches):
    with tf.Graph().as_default():
        ...
        for x in range(batches.shape[2]):
            true_val1 = np.array(range(x * 35, x * 35 + batches.shape[3]))
            assert np.array_equal(batches[0,0,x], true_val1),\
                'Batches returned wrong contents. For example, input sequence {} in the first batch was {} instead of {}'.format(x, batches[0,0,x], true_val1)
             
            true_val2 = np.array(range(x * 35 + 1, x * 35 + 1 + batches.shape[3]))
            assert np.array_equal(batches[0,1,x], true_val2),\
                'Batches returned wrong contents. For example, target sequence {} in the first batch was {} instead of {}'.format(x, batches[0,1,x], true_val2)
